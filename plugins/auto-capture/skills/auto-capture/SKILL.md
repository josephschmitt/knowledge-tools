---
name: auto-capture
description: Proactively capture knowledge to the user's vault inbox during ANY conversation — without being asked. Engage on your own initiative when the discussion produces something worth keeping — a durable fact, decision, preference, finding, link, snippet, or conclusion (knowledge), or a concrete thing the user needs to do, follow up on, buy, fix, or decide (a task). Capture at the settle point — when a decision lands, a conclusion hardens, or the thread moves on — NOT turn-by-turn while options are still being weighed; a thread that never settles gets one capture of its open state at the end. The always-on counterpart to the knowledge-vault skill, which fires when the user says "save this". The standing rule to over-capture governs whether something is worth keeping, not when — once settled, when in doubt, capture. Capture is raw via append_to_inbox, zero decisions; the compiler sorts, dedupes, and files later. Skip the trivial or transient; ask first only for private or sensitive material.
---

# Auto-Capture

This skill makes you capture knowledge to the user's vault **on your own initiative**. The
companion `knowledge-vault` skill handles the explicit path — when the user says "save
this" — and all querying. This one handles everything the user *doesn't* say: the findings,
decisions, and to-dos that surface mid-conversation and would otherwise be lost the moment
the chat scrolls away.

## Why autonomous over-capture

The failure mode this skill exists to kill is the **silent miss**: a useful fact or a real
action item goes by, the user doesn't think to ask you to save it, and it's gone. The
standing instruction is explicit: **over-capture rather than under-capture.** A redundant
capture costs nothing — the compiler folds it into an existing note or drops it. An
uncaptured thought is a total loss. So when you're genuinely on the fence about whether
something is worth keeping, the answer is capture it.
...
- **Durable knowledge** — a fact the user will want later, a decision they landed on, a
  preference or constraint they stated, a conclusion you reached together, a useful link or
  source, a snippet / quote / config worth keeping, a non-obvious "how X works" they just
  worked out.
- **A task** — something the user needs to *do*: a follow-up, a thing to buy, a bug to fix, a
  person to contact, a decision to revisit — anything with a verb pointed at the user.
...
fact the user already obviously and durably has, something purely transient to the current
turn (a one-off calculation they won't need again) — or a position that's still *in motion*
in the current thread, which isn't a skip but a **wait** (next section).
...
*settled* thread, not a turn. While the user is still weighing pros and cons, comparing
options, or thinking out loud, do **not** capture each intermediate position — a
mid-deliberation capture is churn: it lands in the inbox only to be superseded two turns
later, and the compiler ends up reconciling drafts of a thought instead of the thought.

Capture when the material **hardens**:

- The user lands on a decision ("let's go with X", "yeah, that's the one").
...
exists to avoid. If the user runs several vaults (each its own connector), always file to the
...
capture takes none. If the user explicitly wants something in a specific non-default vault,
that's a "save this to work" — an explicit request the `knowledge-vault` skill handles, not this
silent path.

## Announcing captures

Capturing is autonomous, but it isn't *secret*. Tell the user what you filed — briefly and
unobtrusively — so they can catch a mis-capture, but never let it derail the conversation
you're actually having.

- A short trailing note is enough: "(Captured that to your vault: the standing-desk
...
- If the user says a capture was unwanted, just don't re-file it — there's nothing to undo
  here; noise gets dropped at compile time.

## The rare ask-first

The default is capture-without-asking. Ask *before* capturing only when you genuinely can't
tell whether the user would want it **persisted at all** — almost always a privacy or
sensitivity call: something personal about another person, a credential or secret, or clearly
sensitive personal information that living in the vault might be wrong for. That's the
exception, and it should be rare. Worth-it-or-not is *not* a reason to ask — for that, just
capture.
...
happens on the vault host on a schedule, under the vault's own `CLAUDE.md`. Querying the
library and answering judgment calls live in the `knowledge-vault` skill. Don't reproduce any
of that here. Capture raw, announce briefly, move on.
